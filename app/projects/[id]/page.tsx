        ...
        const renderedParts = parts.map((part, pIdx) => {
          ...
          if (part.match(/^\(\[\s*clip:/i) && part.endsWith("])")) {
            ...
            return (
              <span
                key={pIdx}
                className="inline-flex items-center gap-1.5 px-2.5 py-0.5 text-xs font-mono font-medium text-slate-800 dark:text-slate-200 bg-violet-50 dark:bg-violet-950/30 border border-violet-200 dark:border-violet-900/80 rounded-md"
              >
                📋 {clipVal}
              </span>
            );
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const mainImages = project.image ? [project.image] : [fallbackImage];

  return {
    title: `${project.title} — Project`,
    description: desc,
    openGraph: {
      title: `${project.title} — Shaikh Arif`,
      description: desc,
      url: `${siteUrl}/projects/${project._id}`,
      type: "article",
      images: mainImages.map(url => ({
        url,
        width: 1200,
        height: 630,
        alt: project.title,
