    ...
    <div className='py-1 md:py-5 grid grid-cols-1 md:grid-cols-2 gap-10 text-gray-800 dark:text-white/70'>
      ...
      <div className='flex flex-col items-center md:items-start gap-5 md:gap-7 text-center md:text-start'>
        <div>
          ...
          <h1 className='text-[#369483] 
            text-5xl md:text-7xl tracking-normal font-semibold'>Arif Hossain</h1>
        </div>
        <HeroDescription />
        <Link href="/projects">
          <button className='bg-transparent rounded-full border border-gray-500 dark:border-gray-500 font-bold text-gray-800 dark:text-white/70 hover:bg-[#369483] hover:text-white hover:shadow-md px-6 py-2 text-sm cursor-pointer'>My Works</button>
        </Link>
        <SocialLinks />
        {/* <Statistics /> */}
      </div>
